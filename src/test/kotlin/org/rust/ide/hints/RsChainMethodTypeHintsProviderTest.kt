    ...
    fun `test iterator special case`() = doTest("""
        fn main() {
            vec![1, 2, 3]
                ...
                .map(|x/*hint text="[:  i32]"*/| x as u8)/*hint text="[:  [impl  [Iterator [< [Item = u8] >]] ]]"*/
                .map(|x/*hint text="[:  u8]"*/| x as u16)/*hint text="[:  [impl  [Iterator [< [Item = u16] >]] ]]"*/
                .filter(|x/*hint text="[:  [& u16]]"*/| x % 2 == 0)/*hint text="[:  [impl  [Iterator [< [Item = u16] >]] ]]"*/
                .for_each(|x/*hint text="[:  u16]"*/| {})
            ;
        }
    """)
