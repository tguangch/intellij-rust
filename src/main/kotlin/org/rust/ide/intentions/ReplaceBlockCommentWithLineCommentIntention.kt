...
class ReplaceBlockCommentWithLineCommentIntention : RsElementBaseIntentionAction<PsiComment>() {
    ...
    override fun invoke(project: Project, editor: Editor, ctx: PsiComment) {
        ...
        val comments = ctx.text
            .drop(COMMENT_DELIMITER_LEN)
            .dropLast(COMMENT_DELIMITER_LEN)
            .trim()
            .lines()
            .reversed()
        ...
        comments.forEachIndexed { index, comment ->
            ...
            val lineComment = factory.createLineComment(commentText)
            parent.addAfter(lineComment, ctx)
    ...
    companion object {
        const val COMMENT_DELIMITER_LEN: Int = 2 // the length of `/*` or `*/`
    }
}
