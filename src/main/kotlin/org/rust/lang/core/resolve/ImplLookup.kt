...
class ImplLookup(
    ...
    fun selectAllProjectionsStrict(selfTy: Ty, boundTrait: BoundElement<RsTraitItem>): Map<RsTypeAlias, Ty>? = ctx.probe {
        val selection = select(TraitRef(selfTy, boundTrait), recursionDepth = 0).ok() ?: return@probe null
        val assocValues = boundTrait.element.associatedTypesTransitively.associateWith { assocType ->
            lookupAssociatedType(selfTy, selection, assocType)
                ?.let { ctx.normalizeAssociatedTypesIn(it, recursionDepth = 0) }
                ?.withObligations(selection.nestedObligations)
